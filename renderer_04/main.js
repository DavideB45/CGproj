...
Renderer.initializeObjects = function (gl) {
  ...
  loadTexture(Renderer.gl, "./../common/textures/cobblestone_bump.jpg", Renderer.gl.TEXTURE0, gl.RGBA);
...
Renderer.drawSpheres = function (gl, stack){
  ...
  stack.multiply(glMatrix.mat4.fromTranslation(glMatrix.mat4.create(),[0, 1, 0] ));
  gl.uniformMatrix4fv(this.flatShader.uModelMatrxLocation, false, stack.matrix);
  ...
  stack.multiply(glMatrix.mat4.fromTranslation(glMatrix.mat4.create(),[10, 0, 35] ));
  gl.uniformMatrix4fv(this.flatShader.uModelMatrxLocation, false, stack.matrix);
  drawObject(gl, Renderer.sphere, [1, 0.6, 0.2, 1], this.flatShader);
  stack.pop();
...
Renderer.setupAndStart = function () {
  ...
  Renderer.cameras[3] = new FreeCamera()
  Renderer.fanale = 2;
...
on_mouseMove = function(e){
  if(Renderer.currentCamera == 3 && mouse_down){
    dx = e.clientX - mouse_x;
    dy = e.clientY - mouse_y;
    mouse_x = e.clientX;
    mouse_y = e.clientY;
    Renderer.cameras[3].dmouse = [dx, dy];
  }
}
on_keyup = function(e){
	Renderer.car.control_keys[e.key] = false;
  if(Renderer.currentCamera == 3)
    Renderer.cameras[3].keyPress[e.key] = false;
}
on_keydown = function(e){
	Renderer.car.control_keys[e.key] = true;
  if(Renderer.currentCamera == 3)
    Renderer.cameras[3].keyPress[e.key] = true;
}
...
update_mode = function(e){
  Renderer.cameras[3].mode = parseInt(e);
}
